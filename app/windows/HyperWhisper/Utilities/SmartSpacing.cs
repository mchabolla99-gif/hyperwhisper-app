...
public static class SmartSpacing
{
    ...
    public static string RemoveFillerWords(string text)
    {
        ...
        //
        // Deliberately requires whitespace on at least one side (no bare "matches the
        // entire string" alternative) - this function is also reachable from the batch
        // path (TranscriptionOrchestrator), which has no language gate. A standalone-match
        // alternative here could fully erase a legitimate single-word non-English batch
        // transcript (e.g. German "er"). The streaming-only case of a confirmed delta that
        // is ENTIRELY a filler word is handled locally at the streaming call site
        // (StreamingTranscriptionClient.AppendFinalTranscript), which is already gated to
        // English, instead of widening this shared regex.
